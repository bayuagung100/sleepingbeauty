...
if(!empty($_POST["id"])){
    ...
    $query = $mysqli->query("SELECT COUNT(*) as num_rows FROM product WHERE id < ".$_POST['id']." AND product_category LIKE '%$_POST[name]%' ORDER BY id DESC");
    ...
    $pquery = $mysqli->query("SELECT * FROM product WHERE id < ".$_POST['id']." AND product_category LIKE '%$_POST[name]%' ORDER BY id DESC LIMIT $showLimit");
    $jml = $pquery->num_rows;
    while ($alldata = $pquery->fetch_array()) {
        ...
        $katquery = $mysqli->query("SELECT * FROM category_product WHERE id LIKE '%$kategori%' ");
    ...
    if($totalRowCount > $showLimit){
        echo '
        <!-- Load more -->
        <div id="more_category_main' . $id . '" class="flex-c-m flex-w w-full p-t-45">
            <button id="' . $id . '" name="'.$ik.'" class="flex-c-m stext-101 cl5 size-103 bg2 bor1 hov-btn1 p-lr-15 trans-04 more_category">
                Load More
            </button>
            <button class="flex-c-m stext-101 cl5 size-103 bg2 bor1 hov-btn1 p-lr-15 trans-04 loading_category" style="display:none">
                Loading ...
            </button>
        </div>
        ';
